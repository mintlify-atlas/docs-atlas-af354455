---
title: 'Write and edit with GitBook Agent'
description: 'Use GitBook Agent to generate and build content for your documentation'
---
...
You can add your own style guide and custom instructions at an organization level — so the Agent will always match your tone and style whenever you ask it to help.
</Info>
...
Here you can write a prompt for the Agent to follow — it will explain what it's doing as it follows your instructions, with the changes appearing in your space as it works.
...
<Steps>
  <Step title="Write your prompt">
    Describe what you want the Agent to do in the change request.
  </Step>
  <Step title="Add reference pages">
    Include any pages that might provide useful context for the changes.
  </Step>
  <Step title="Start the change request">
    Click **Start change request** and the Agent will open a change request for you and start carrying out your instructions.
  </Step>
...
At every stage, the Agent will tell you what it's doing in the chat window on the right-hand side of the app.
...
<CardGroup cols={2}>
  <Card title="Break it down" icon="list">
    The Agent is best at completing focused tasks. Break down complex projects into smaller steps and ask the Agent to complete them one at a time.
  </Card>
  
  <Card title="Be specific" icon="crosshairs">
    Generic prompts like `@gitbook improve this page` will apply general best practices, but without more specific guidance the Agent may not achieve the goal you had in mind.
  </Card>
  
  <Card title="Focus on outcomes" icon="bullseye">
    If you're hearing about a specific problem customers are having, tell the Agent about those problems — or the outcomes you want to achieve. It will suggest improvements based on those outcomes.
  </Card>
  
  <Card title="Give direct instructions" icon="arrow-right">
    If you want the Agent to use a stepper block for a step-by-step guide, or add an FAQ section with a bunch of expandable blocks, tell it precisely what to do to get the right results first time.
  </Card>
</CardGroup>

<Tip>
**Use broad prompts for wider improvements**

For maintenance tasks like fixing typos, updating a feature name across pages or removing specific block types from your docs, you can use commands like `@gitbook replace all instances of v2.3.9 with v2.4.0`.
</Tip>
...
```
Create a quickstart guide for our API with authentication steps
```

```
Update all code samples on this page to use Python 3.11 syntax
```

```
Add a troubleshooting section with common errors and solutions
```

```
Rewrite this page to be more concise and action-oriented
```

```
Create a comparison table showing our pricing tiers and features
```